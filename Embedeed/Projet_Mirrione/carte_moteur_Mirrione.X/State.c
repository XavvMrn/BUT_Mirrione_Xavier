...
unsigned char stateRobot, autoControlActivated = 1;
...
void SetNextRobotStateInAutomaticMode() {
    ...
    if (nextStateRobot != stateRobot - 1) {
        if(autoControlActivated == 1) {
            ...
            payload[0] = (unsigned char) stateRobot;
            payload[1] = (unsigned char) (timestamp >> 24); // 4 premier octet 
            payload[2] = (unsigned char) (timestamp >> 16); // 2 autres 
            payload[3] = (unsigned char) (timestamp >> 8); // dernier
